...
if __name__ == "__main__":
    ...
    if Path(kpis_v2_file).exists():
        print(f"Using KPIs v2 data: {kpis_v2_file}")
        data_file = kpis_v2_file
    elif Path(original_data_file).exists():
        print(f"Using original KPI data: {original_data_file}")
        data_file = original_data_file
    else:
        print("\n[WARNING] No data file found!")
        print(f"Please add your data to either:")
        print(f"  - {kpis_v2_file} (processed from kpis_v2.csv)")
        print(f"  - {original_data_file} (direct format)")
        print("\nExpected CSV format:")
